...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it(`(multiparty) can allow access to setOperatingStatus() for Contract Owner account`, async function () {
      ...
      try 
      {
          await config.flightSuretyData.setOperatingStatus(false, { from: config.ownerAirline });
      }
      catch(e) {
          reverted = true;
      }
  ...
  it(`(multiparty) can block access to functions using requireIsOperational when operating status is false`, async function () {
      ...
      try
      {
          await config.flightSuretyApp.testIsBool(true, { from: config.ownerAirline });
  ...
  it('(airline) cannot register an Airline using registerAirline() if it is not funded', async () => {
    ...
    // ACT
    try {
        await config.flightSuretyApp.registerAirline(newAirline, {from: config.ownerAirline});
    }
    catch(e) {
